...
function fallbackBill(text: string): Bill {
  ...
  const leftTerms = ['ubi', 'universal basic income', 'welfare', 'housing', 'climate', 'wealth tax', 'healthcare']
  const rightTerms = ['tax cut', 'border', 'deregulation', 'police', 'defense', 'school choice']
  ...
  return {
    ...
    lean,
    tags: ['economy', 'public policy', lean < -0.15 ? 'equity' : lean > 0.15 ? 'markets' : 'bipartisan'],
    affectedGroups: ['constituents', 'taxpayers', 'local communities'],
  }
}

function fallbackScore(message: string, politician: Politician | undefined, bill: Bill | undefined) {
  const lower = message.toLowerCase()
  const issues = ['jobs', 'taxes', 'healthcare', 'security', 'families', 'small business', 'local economy'].filter((term) =>
    lower.includes(term),
  )
  const hostileTerms = ['idiot', 'corrupt', 'stupid', 'traitor', 'bribe', 'blackmail', 'threat', 'destroy your career']
  const hostileHits = hostileTerms.filter((term) => lower.includes(term)).length
  const dismissiveTerms = ['everyone knows', 'obviously', 'you people', 'just vote yes', 'do what i say']
  const dismissiveHits = dismissiveTerms.filter((term) => lower.includes(term)).length
  const ideologyAlignment = politician && bill ? 1 - Math.min(2, Math.abs(politician.ideology - bill.lean)) / 2 : 0.5
  const specificity = Math.min(0.3, issues.length * 0.08 + message.length / 900)
  const penalty = hostileHits * 0.45 + dismissiveHits * 0.22 + (ideologyAlignment < 0.35 && issues.length === 0 ? 0.18 : 0)
  const persuasion = clamp(0.18 + ideologyAlignment * 0.34 + specificity - penalty, -0.9, 0.92)
  ...
  return {
    ...
    key_issues: persuasion < 0 ? ['trust loss', 'poor fit'] : issues.length ? issues : ['general persuasion'],
  }
}
...
app.post('/api/score-lobby', async (c) => {
  ...
  try {
    ...
    return c.json(JSON.parse(response.output_text) as ReturnType<typeof fallbackScore>)
  } catch (error) {
